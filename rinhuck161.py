...
Original file is located at
    https://colab.research.google.com/drive/138LpyCCnCV26Kf0uYLCAKmcbLV2zS1_o
"""
import gc
...
def augment_tokenize_python_code(python_code_str, mask_factor=0.3):
    ...
    var_dict = {} # Словарь, в котором хранятся замаскированные переменные

    # определенные зарезервированные слова, которые не следует рассматривать как обычные переменные и
    # следовательно, необходимо исключить из наших дополнений к переменной маске
    ...
    for i in range(0, len(python_tokens)):
      if python_tokens[i].type == 1 and python_tokens[i].string not in skip_list:
        ...
        if i>0 and python_tokens[i-1].string in ['def', '.', 'import', 'raise', 'except', 'class']: # avoid masking modules, functions and error literals
          ...
          tokenized_output.append((python_tokens[i].type, python_tokens[i].string))
        elif python_tokens[i].string in var_dict:  # если переменная уже замаскирована
          tokenized_output.append((python_tokens[i].type, var_dict[python_tokens[i].string]))
        elif random.uniform(0, 1) > 1-mask_factor: # переменные случайной маски
...
np.random.seed(0)
msk = np.random.rand(len(python_problems_df)) < 0.85 # Разделение данных на 85% подготовки и 15% проверки
...
model.load_state_dict(torch.load(r'C:\Users\alexe\PycharmProjects\rinhak\model.pt'))
...
TRG = Output
gc.collect()
src = "function to sort a list of dictionaries by key"
